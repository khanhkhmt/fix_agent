...
class UIController {
        ...
        if (dialBtn) {
            dialBtn.addEventListener('click', () => {
                ...
                if (isConnected) {
                    ...
                    this.addChatMessage('Đã ngắt kết nối, hẹn gặp lại bạn lần sau~😊', false);
                } else {
                    ...
                    if (!otaUrlInput || !otaUrlInput.value.trim()) {
                        ...
                        this.addChatMessage('Vui lòng điền URL máy chủ OTA', false);
                        return;
                    }
                        ...
                        if (typeof window.startCamera === 'function') {
                            window.startCamera().then(success => {
                                if (success) {
                                    ...
                                    this.addChatMessage('⚠️ Khởi động camera thất bại, vui lòng kiểm tra quyền truy cập trình duyệt', false);
                                }
                            }).catch(error => {
    ...
    updateDialButton(isConnected) {
        ...
        if (dialBtn) {
            if (isConnected) {
                ...
                dialBtn.querySelector('.btn-text').textContent = 'Bắt đầu';
        ...
        if (recordBtn) {
            ...
            if (isConnected && microphoneAvailable) {
                ...
                if (!microphoneAvailable) {
                    recordBtn.title = window.isHttpNonLocalhost ? 'Hiện tại do truy cập qua HTTP, không thể ghi âm, chỉ có thể tương tác bằng văn bản' : 'Micro không khả dụng';
                } else {
    ...
    startAIChatSession() {
        ...
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage('⚠️ Hiện tại do truy cập qua HTTP, không thể ghi âm, chỉ có thể tương tác bằng văn bản', false);
            } else {
                this.addChatMessage('⚠️ Micro không khả dụng, vui lòng kiểm tra cài đặt quyền truy cập, chỉ có thể tương tác bằng văn bản', false);
            }
        }
    ...
    async handleConnect() {
        ...
        this.addChatMessage('Đang kết nối đến máy chủ...', false);
